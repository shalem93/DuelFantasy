...
struct PlayoffTiersGroupDetailView: View {
    @Bindable var viewModel: PlayoffTiersViewModel
    let group: PlayoffTiersGroup
    ...
    private var actionButtons: some View {
        VStack(spacing: 8) {
            NavigationLink {
                ChatRoomView(leagueId: "group-playoff_tiers-\(group.id.uuidString)", title: "\(group.name) Chat")
            } label: {
    ...
    // Mirrors PlayoffTiersLiveView so group rows match the main leaderboard.
