...
struct PlayoffTiersLiveView: View {
    @Bindable var viewModel: PlayoffTiersViewModel
    @State private var refreshTick: Int = 0
    @State private var selectedEntry: PlayoffTiersLeaderboardEntry?
    ...
    private func pickRow(tier: Int, player: PlayoffTiersPlayer) -> some View {
        let points = viewModel.livePlayerPoints[player.id] ?? 0
        let isEliminated = viewModel.eliminatedTeams.contains(player.team)
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text("\(player.team) \u{2022} \(player.position)")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    private func leaderboardRow(_ entry: PlayoffTiersLeaderboardEntry) -> some View {
        HStack {
            Text("\(entry.rank)")
    ...
    private func entryDetailSheet(_ entry: PlayoffTiersLeaderboardEntry) -> some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    // Rank & Points header
                    ...
                    VStack(spacing: 0) {
                        ForEach(entry.picks.sorted(by: { $0.tier < $1.tier }), id: \.tier) { pick in
                            ...
                            let isEliminated = viewModel.eliminatedTeams.contains(pick.playerTeam)
                            HStack(spacing: 10) {
                                Text("T\(pick.tier)")
                                    .font(.system(size: 11, weight: .bold))
                                    .foregroundStyle(.white)
