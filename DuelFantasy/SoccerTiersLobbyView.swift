...
struct SoccerTiersLobbyView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            if viewModel.isLoading && !viewModel.hasAttemptedLoad {
                ...
                SoccerTiersLiveView(viewModel: viewModel)
    ...
    private func playerRow(player: SoccerTiersPlayer, isSelected: Bool, rank: Int) -> some View {
        Button {
            Haptics.light()
            if isSelected {
    ...
    private func groupRow(_ group: SoccerTiersGroup) -> some View {
        HStack(spacing: 12) {
    ...
    private var createGroupSheet: some View {
        NavigationStack {
            VStack(spacing: 20) {
                Text("Create a private group to compete with friends in World Cup Tiers.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
                    .padding(.horizontal)
