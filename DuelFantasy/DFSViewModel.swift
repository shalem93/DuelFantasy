...
final class DFSViewModel {
    ...
    // MARK: - Bot generation (DFSBotGenerator, off the main actor)

    /// Lock-guarded pools cache shared by every generator snapshot this VM
    /// hands out — survives across batches and pre-cache task-group members.
    @ObservationIgnored private let botGenPoolsCache = DFSBotGenPoolsCache()

    /// Snapshot everything bot generation reads from this VM so the
    /// generation itself (DFSBotGenerator, nonisolated) can run off the main
    /// actor. Cheap: a handful of value copies.
    func makeBotGenerator() -> DFSBotGenerator {
        let started = Set(slateGames.map(\.id).filter { gameHasStarted($0) })
        return DFSBotGenerator(
            sport: sport, activeTournamentID: activeTournamentID, poolRevision: poolRevision,
            slateGames: slateGames, startedGameIDs: started, poolsCache: botGenPoolsCache
        )
    }

    /// Whether a player matches a roster slot requirement (see DFSBotGenerator).
    private func playerMatchesSlot(_ player: DFSPlayer, slot: String, isSingleGame: Bool = false) -> Bool {
        DFSBotGenerator.playerMatchesSlot(player, slot: slot, isSingleGame: isSingleGame, sport: sport)
    }

    /// Synchronous single-lineup generation ON the main actor. Fine for a
    /// one-off (late-swap repair of a single bot); field-sized loops must use
    /// `generateBotLineupsOffMain` — 2000 of these on a big pool is a watchdog kill.
    private func generateBotLineup(from players: [DFSPlayer], salaryCap: Int, lineupSize: Int, rosterSlots: [String]? = nil, isSingleGame: Bool = false, sportOverride: String? = nil, settlementCacheToken: String? = nil) -> [String] {
        makeBotGenerator().generateBotLineup(
            from: players, salaryCap: salaryCap, lineupSize: lineupSize, rosterSlots: rosterSlots,
            isSingleGame: isSingleGame, sportOverride: sportOverride, settlementCacheToken: settlementCacheToken
        )
    }

    /// Generate `count` bot lineups OFF the main actor, in batches. The main
    /// thread only runs the cheap per-batch `onBatch` (append + publish), so
    /// SwiftUI keeps painting while a 2000-bot field builds. Batches arrive in
    /// order; the returned array is every lineup generated.
    func generateBotLineupsOffMain(
        count: Int, from players: [DFSPlayer], salaryCap: Int, lineupSize: Int,
        rosterSlots: [String]? = nil, isSingleGame: Bool = false,
        sportOverride: String? = nil, settlementCacheToken: String? = nil,
        batchSize: Int = 50,
        onBatch: ([[String]]) -> Void = { _ in }
    ) async -> [[String]] {
        ...
        while produced < count {
            let n = min(batchSize, count - produced)
            let batch: [[String]] = await Task.detached(priority: .userInitiated) {
                (0..<n).map { _ in
                    gen.generateBotLineup(
                        from: players, salaryCap: salaryCap, lineupSize: lineupSize, rosterSlots: rosterSlots,
                        isSingleGame: isSingleGame, sportOverride: sportOverride, settlementCacheToken: settlementCacheToken
                    )
                }
    ...
    /// Settlement bot generation off the main actor: each bot drafts from a
    /// freshly salary-implied (hindsight-blind) projection roll of the pool,
    /// with the duplicate-lineup re-roll. Returns each lineup with the salary
    /// map of the pool it drafted from (what the result rows record).
    private func generateSettlementBotsOffMain(
        count: Int,
        basePlayers: [(id: String, name: String, salary: Int, actualPoints: Double, position: String, hasRealSalary: Bool)],
        battingOrders: [String: Int],
        salaryCap: Int, lineupSize: Int, rosterSlots: [String]?, isSingleGame: Bool,
        sportOverride: String, avgPoints: Double,
        seenLineupKeys: Set<String>
            ...
            for _ in 0..<count {
                let pool: [DFSPlayer] = basePlayers.map { p in
                    ...
                    var player = DFSPlayer(
                        ...
                        battingOrder: battingOrders[p.id]
                    )
                    player.isConfirmedActive = p.hasRealSalary
                    return player
                }
                var ids = gen.generateBotLineup(from: pool, salaryCap: salaryCap, lineupSize: lineupSize, rosterSlots: rosterSlots, isSingleGame: isSingleGame, sportOverride: sportOverride, settlementCacheToken: settleToken)
                // Duplicate-lineup re-roll: identical builds collapse field
                // variance — try a few fresh rolls before accepting a repeat.
                var rerolls = 0
                while rerolls < 3, seen.contains(ids.sorted().joined(separator: "|")) {
                    ids = gen.generateBotLineup(from: pool, salaryCap: salaryCap, lineupSize: lineupSize, rosterSlots: rosterSlots, isSingleGame: isSingleGame, sportOverride: sportOverride, settlementCacheToken: settleToken)
                    rerolls += 1
                }
                seen.insert(ids.sorted().joined(separator: "|"))
                let salaries = Dictionary(pool.map { ($0.id, $0.salary) }, uniquingKeysWith: { a, _ in a })
                out.append((ids: ids, salaries: salaries))
            }
            return out
        }.value
    }
        ...
        if !fieldGenerated {
            ...
            if let savedBots, !savedBots.isEmpty {
                ...
                if staleVsConfirmedLineups || malformedGeneration {
                    ...
                    _ = await generateBotLineupsOffMain(count: botsToGenerate, from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame) { batch in
                        for newIDs in batch {
                            let index = freshBots.count
                    ...
                    let regenerated = await generateBotLineupsOffMain(count: invalidBots.count, from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame)
                    for (bot, newIDs) in zip(invalidBots, regenerated) {
                    ...
                    var regenerated = await generateBotLineupsOffMain(count: wrongShapeBots.count, from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame).makeIterator()
                    for bot in trimmedBots {
                        let key = "\(bot.name)|\(bot.playerIDs.joined(separator: ","))"
                        if wrongShapeIDs.contains(key) {
                            let newIDs = regenerated.next() ?? generateBotLineup(from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame)
                            botFieldEntries.append(DFSFieldEntry(id: UUID(), name: bot.name, playerIDs: newIDs, isCurrentUser: false))
                        } else {
                ...
                if totalNonUser < targetBots && !activePlayers.isEmpty
                    ...
                    var padded = 0
                    _ = await generateBotLineupsOffMain(count: botsNeeded, from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame) { batch in
                        var appended = fieldEntries
                        for botPlayerIDs in batch {
                            let i = padded
                            let baseName = sampleNames[(startIndex + i) % sampleNames.count]
                            let uniqueName = "\(baseName) #\(startIndex + i + 1)"
                            appended.append(DFSFieldEntry(id: UUID(), name: uniqueName, playerIDs: botPlayerIDs, isCurrentUser: false))
                            padded += 1
                        }
                        fieldEntries = appended
                    }
                    needsResave = true
                }
                ...
                _ = await generateBotLineupsOffMain(count: count, from: activePlayers, salaryCap: tournament.salaryCap, lineupSize: tournament.lineupSize, rosterSlots: tournament.rosterSlots, isSingleGame: tournament.isSingleGame) { batch in
                    for botPlayerIDs in batch {
                        if botPlayerIDs.isEmpty { emptyCount += 1 }
                        let index = accumulated.count
                        let baseName = sampleNames[index % sampleNames.count]
    ...
    private func preCacheSingleTournament(tid: String, token: String, userID: String) async {
        ...
        do {
            ...
            if !savedBots.isEmpty && savedBotsAreValid {
                ...
                // Generation runs off the main actor in batches; each batch
                // lands here on the main actor to append + (once) publish.
                var publishedReady = false
                var generatedCount = 0
                _ = await generateBotLineupsOffMain(count: botsNeeded, from: poolForBots, salaryCap: tObj.salaryCap, lineupSize: tObj.lineupSize, rosterSlots: tObj.rosterSlots, isSingleGame: tObj.isSingleGame) { batch in
                    for botIDs in batch {
                        let i = generatedCount
                        generatedCount += 1
        ...
        if let savedBots = savedBotField, !savedBots.isEmpty {
            ...
            if currentBotCount < targetBots {
                ...
                let generated = await generateSettlementBotsOffMain(
                    ...
                    isSingleGame: isSingleGame, sportOverride: sportPrefix.uppercased(),
                    avgPoints: avgPoints, seenLineupKeys: seenLineupKeys
                )
                for (i, made) in generated.enumerated() {
                    let botPlayerIDs = made.ids
                    ...
                    let botPlayerLookup = made.salaries
